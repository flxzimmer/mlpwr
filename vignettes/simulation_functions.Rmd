...
simfun_anova = function(n,n.groups) {
    ...
    # Generate a data set
    groupmeans = rnorm(n.groups,sd = .2) # generate groupmeans using cohen's f=.2
    dat = sapply(groupmeans,function(x) rnorm(n,mean=x,sd=1)) # generate data
    dat = dat |> as.data.frame() |> gather() # format
...
mod.original <- glm(counts ~ outcome + treatment,data=dat.original, family = poisson)
...
simfun_glm1 = function(N) {
    ...
    # generate data
    dat = data.frame(
      outcome = gl(3,1,ceiling(N/3)),
      treatment = gl(3,ceiling(N/3))
    )[1:N,]
    a = predict(mod.original,newdata=dat,type="response")
    dat$counts= rpois(N,a)
   
  # test hypothesis
    mod <- glm(counts ~ outcome + treatment,data=dat, family = poisson)
    summary(mod)$coefficients["treatment2","Pr(>|z|)"]<.01
}
```
...
simfun_glm2 = function(N) {
  ...
  # test hypothesis
    mod <- glm(criterion ~ pred1+pred2,data=dat, family = binomial)
...
simfun_irt2 = function(N1,N2) {
  ...
  # fit models
    ...
    mod2 = multipleGroup(dat,1, group=group,invariance=c("slopes", "intercepts"))
...
simfun_multi1 = function(N) {
  ...
  dat$x = simulate(~period + (1 | herd),newdata=dat, family=poisson, newparams=params)[[1]]


  # test hypothesis
  mod <- glmer(x ~ period + (1 | herd),
             data = dat, family = poisson) # fit model
...
dat.original = data.frame(
  country = rep(1:n.countries.original,length.out=N.original),
  pred1 = rnorm(N.original),
  pred2 = rnorm(N.original)
)
country.intercepts= rnorm(n.countries.original,sd=.5)
dat.original$intercepts = country.intercepts[dat.original$country]
beta = c(1,.4,-.3) # parameter weights
prob = logistic( as.matrix(dat.original[c("intercepts","pred1","pred2")]) %*% as.matrix(beta)) # get probability
dat.original$criterion = runif(N.original)<prob # draw according to probability

# fit original model to obtain parameters
mod.original = glmer(criterion ~ pred1 + pred2 + 0 + (1|country),data = dat.original,family=binomial)
```
...
simfun_multi2 = function(n,n.countries) {
  ...
  # generate data
  dat = data.frame(
  country = rep(1:n.countries,length.out=n*n.countries),
  pred1 = rnorm(n*n.countries),
  pred2 = rnorm(n*n.countries)
)
  dat$criterion = simulate(mod.original,nsim=1,newdata = dat,allow.new.levels = TRUE,use.u = FALSE) |> unlist() # criterion data from the fitted model

  # test hypothesis
  mod = glmer(criterion ~ pred1 + pred2 + 0 + (1|country),data = dat,family=binomial)
  summary(mod)[["coefficients"]]["pred2","Pr(>|z|)"]<.01
  }
```
...
```{r}
costfun_multi2 = function(n,n.countries) 5*m + 100*n.countries
```
