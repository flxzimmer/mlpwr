...
svm.fit = function(dat,lastfit,tune=TRUE) {
  ...
  if (tune) {
    ctrl = WeightSVM::tune.control(cross=min(10,nrow(datx)),error.fun=logloss)
    a = WeightSVM::tune_wsvm(y ~ .,data = datx,weight=weight,ranges=tunepars,tunecontrol=ctrl)
  ...
  if (!tune) {
    mod = WeightSVM::wsvm(y ~ .,data = datx,cost=pars$cost,epsilon=pars$epsilon,gamma=pars$gamma,weight=weight)
  }

  # Save used parameters
  pars_history = rlist::list.append(pars_history,as.data.frame(pars))
...
gauss.fit = function(dat,patience=100,control) {
  ...
  re = list(fitfun=fitfun,fitfun.sd=fitfun.sd,badfit=badfit)
