...
get.pred =function(fit,dat,power,costfun,cost,boundaries,Ntry=20,task) {
  ...
  if (task == "desiredpower"){
    ...
    fn = function(x) relu(power-fit$fitfun(x)-fit$fitfun.sd(x)*greediness/10)*10^5+costfun(x)/costfun(midpars)

    if(is.null(fit$fitfun.sd)) fn = function(x) relu(power-fit$fitfun(x))*10^5+costfun(x)/costfun(midpars)

      newre = rgenoud::genoud(fn,nvars=nrow(Domains),data.type.int=TRUE,Domains = Domains,print.level = 0,boundary.enforcement=1,pop.size = 20,starting.values = cands)


    badprediction = abs(fit$fitfun(newre$par)-power)>.05
  }


  if (task == "costthreshold" ){

    fn = function(x) relu(costfun(x)-cost)/cost*10^5-fit$fitfun(x)
  ...
  re = list(points=points, badprediction=badprediction,edgeprediction=edgeprediction)
