...
simfun_anova = function(n,n.groups) {
  ...
  # Generate a data set
  groupmeans = stats::rnorm(n.groups,sd = .2) # generate groupmeans using cohen's f=.2
  dat = sapply(groupmeans,function(x) stats::rnorm(n,mean=x,sd=1)) # generate data
  dat = dat |> as.data.frame() |> tidyr::gather() # format

  # Test the hypothesis
  res <- stats::aov(value ~ key, data = dat) # perform ANOVA
  summary(res)[[1]][1,5]<.01   # extract significance
}
...
simfun_glm2 = function(N) {
  ...
  beta = c(1.2,.8) # parameter weights
  prob = logistic(as.matrix(dat) %*% beta) # get probability
  dat$criterion = stats::runif(N)<prob # draw according to probability

  # test hypothesis
  mod <- stats::glm(criterion ~ pred1+pred2,data=dat, family = stats::binomial)
  summary(mod)$coefficients["pred2","Pr(>|z|)"]<.01
}
...
simfun_irt1 = function(N) {
  ...
  # generate data
  dat = mirt::simdata(
    a = c(1.04, 1.20, 1.19, 0.61, 1.31, 0.83, 1.46, 1.27, 0.51, 0.81),
    d = c(0.06, -1.79, -1.15,  0.88, -0.20, -1.87,  1.23, -0.08, -0.71, 0.60),
    N = N,
    itemtype = "2PL"
  )
  ...
  res$p[2]<.01 # extract significance
}
...
simfun_irt2 = function(N1,N2) {
  ...
  # generate data
  a1 = a2 =  c(1.04, 1.20, 1.19, 0.61, 1.31, 0.83, 1.46, 1.27, 0.51, 0.81)
  d1 = d2 = c(0.06, -1.79, -1.15,  0.88, -0.20, -1.87,  1.23, -0.08, -0.71, 0.60)
  a2[1] = a2[1]+.3
  d2[1] = d2[1]+.5
  dat1 = mirt::simdata(
    a = a1,
  ...
  mod2 = mirt::multipleGroup(dat,1, group=group,invariance=c("slopes", "intercepts"))
...
simfun_multi1 = function(N) {
  ...
  # generate data
  params <- list(theta=0.5,beta=c(2,-.2,-.4,-.6))
  dat <- expand.grid(herd=1:ceiling(N/4),period=factor(1:4))[1:N,]
  dat$x = lme4::simulate.formula(~period + (1 | herd),newdata=dat, family=stats::poisson, newparams=params)[[1]]


  # test hypothesis
  mod <- lme4::glmer(x ~ period + (1 | herd),
               data = dat, family = stats::poisson) # fit model
  pvalues = summary(mod)[["coefficients"]][2:4,"Pr(>|z|)"]
  any(pvalues<.01)
}
